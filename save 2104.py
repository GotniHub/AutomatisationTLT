...
def display_customer_report(data_plan_prod, data_float, rates):
    ...
    if fichier:
        ...
        try:
            ...
            df_formations = df_formations.dropna(subset=["Date de début", "Nombre de jour"])
            ...
            jours_consulting = max(jours_consulting, 0)  # éviter négatif
            #st.write("\n")
    ...
    # 🔁 CA consulting = Total - Formation
    ca_engage_consulting = ca_engage_total - ca_engage_formateurs
    ca_engage_consulting = max(ca_engage_consulting, 0)

    # 📌 Totaux généraux
    mission_logged_days = intervenants["Jours Réalisés"].sum()
    jours_consulting = mission_logged_days - sum(jours_formation_par_formateur.values())

    # 🧮 Budget & ratios
    mission_budget = final_plan_prod['Budget (PV)'].sum()
    budget_remaining = mission_budget - ca_engage_consulting
    percentage_budget_used = (ca_engage_consulting / mission_budget) * 100 if mission_budget else 0
    ...
    st.metric("CA Engagé Consulting", f"{ca_engage_consulting:,.0f} €")
    ...
    with col3 : 
        ...
        """, unsafe_allow_html=True)
            # ✅ Nouveau calcul basé uniquement sur les jours de consulting
    ...
    with col6:
        ...
        st.subheader("📊 Répartition Jours : Formation vs Consulting")
        data = pd.DataFrame({
            "Type": ["Formation", "Consulting"],
            "Jours": [jours_formation, jours_consulting]
        })

        fig = px.pie(data, names="Type", values="Jours", title="Part des jours consacrés aux Formations vs Consulting",
