...
def display_customer_report(data_plan_prod, data_float, rates):
    ...
    if "Module" in df_formations.columns:
        modules_disponibles = df_formations["Module"].dropna().unique().tolist()
        modules_disponibles.sort()
        module_selectionne = st.sidebar.multiselect("📚 Filtrer par Module", options=modules_disponibles, default=modules_disponibles)
    ...
    intervenants_selectionnes = st.sidebar.multiselect(
    ...
    # st.metric("CA Engagé Total", f"{ca_engage_total:,.0f} €")
    # st.metric("CA Engagé Formation", f"{ca_engage_formateurs:,.0f} €")
    # st.metric("CA Engagé ingenieurie", f"{ca_engage_ingenieurie:,.0f} €")
    ...
    with col1:
        ...
        # 🔹 Ajouter une colonne temporaire "is_formateur" pour style
        ...
        styled_df = tableau_cumul_jours_ingenierie.style.apply(highlight_formateur, axis=1)
        ...
        # 🔹 Ajouter une colonne temporaire "is_formateur"
        tableau_cumul_jours_ingenierie['is_formateur'] = tableau_cumul_jours_ingenierie['Acteur'].apply(
            lambda x: clean_nom(x) in formateurs_clean if pd.notna(x) else False
        )

        # 🔹 Formateurs visibles avec jours d’ingénierie > 0
        formateurs_visibles = tableau_cumul_jours_ingenierie[
            tableau_cumul_jours_ingenierie['is_formateur'] & 
    ...
    with col2:
        ...
        def highlight_ca_formateur(row):
            return ['background-color: #FFF2CC' if row.get('Est_formateur', False) else '' for _ in row]

        styled_ca_df = tableau_cumul_ca_ingenierie.style.apply(highlight_ca_formateur, axis=1)
    ...
    with col1: 
        ...
        # 🟨 Ajout colonne is_formateur
        intervenants["is_formateur"] = intervenants["Acteur"].apply(
            lambda x: clean_nom(x) in formateurs_clean if pd.notna(x) else False
        )
        ...
        def highlight_formateurs(row):
            return ['background-color: #FFF2CC' if row.get('is_formateur', False) else '' for _ in row]
